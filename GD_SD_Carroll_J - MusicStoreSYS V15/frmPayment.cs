...
namespace MusicStoreSYS
{
    public partial class frmPayment : Form
    {
        ...
        private void btnAcceptSupp_Click(object sender, EventArgs e)
        {
           ...
           // +"Payment Date: " + mthPaymentDate.SelectionRange.Start.ToShortDateString()
            ...
            MessageBox.Show("The Supplier:  " + cboPaymentCD.GetItemText(cboPaymentCD.SelectedItem) + "\nPayment ID: " + txtPaymentID.Text + 
                "\nOrder ID: " + txtOrderID.Text + " has paid for the CDs on \nPayment Date: " + 
                mthPaymentDate.SelectionRange.Start.ToShortDateString() + "\nThe Amount paid is: €" + txtAmount.Text, 
        ...
        public void loadOrderItems()
        {
            ...
            for (int i = 0; i < DS.Tables["ss"].Rows.Count; i++)
                cboOrderID.Items.Add(DS.Tables[0].Rows[i][0].ToString().PadLeft(4, '0') + " " + String.Format("{0:dd-MMM-yy}",DS.Tables[0].Rows[i][1]) + " " + DS.Tables[0].Rows[i][2].ToString() + " " + DS.Tables[0].Rows[i][3].ToString().PadLeft(3,'0'));
