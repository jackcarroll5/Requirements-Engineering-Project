...
namespace MusicStoreSYS
{
    public partial class frmRecCD : Form
    {
        ...
        private void btnSelSupplier_Click(object sender, EventArgs e)
        {
            ...
            {
                Order.receiveOrder(Convert.ToInt32(cboSuppliersPreOrder.Text.Substring(0,4)));
        ...
        private void cboSuppliersPreOrder_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < DS.Tables["ss"].Rows.Count; i++)
                cboCDChoice.Items.Add(DS.Tables[0].Rows[i][0].ToString().PadLeft(4, '0') + " " + DS.Tables[0].Rows[i][1].ToString() + " " + DS.Tables[0].Rows[i][2].ToString() + " " + DS.Tables[0].Rows[i][3].ToString());
